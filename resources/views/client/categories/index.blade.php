        ...
        <div class="card-body">
            <div class="row">
                @foreach ($categories as $category)
                    <a class="col mt-1 text-decoration-none"
                        href="{{ route('products.index', ['username' => $user->username, 'categorySlug' => $category->slug]) }}">
                            ...
                            @if ($category->files->count() > 0)
                                @foreach ($category->files as $key => $image)
                                    <img data-fancybox="gallery-{{ $category->id }}" data-caption="{{ $category->name }}"
                                        ...
                                        class="card-img-top img-fluid @if ($key != 0) d-none @endif"
                                        alt="{{ $category->name }}">
                                @endforeach
                            @else
